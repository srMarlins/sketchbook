...
### Migration `9.sqm` (added during PR review)

User flagged that the same orphaning bug class showed up on the History column even after the above. Cause: `MacPathRepaired` (and similar payload-less actions) carry no path, so the resolver's `pathHint` fallback had nothing to chew on once the row's `project_name` was NULL. Proposals had solved this by carrying `name + path` in their args JSON; we mirror that with row-level denorm.

`9.sqm` adds `project_path TEXT` to `journal_entries` and backfills it the same way 8.sqm backfills `project_name`. `SqlJournalRepository.append` resolves both name+path from `selectProjectById` at write time; callers (notably `SqlRepairRepository.applyMacPathRepair` / `restoreMacPathRepair`) pre-fill them from their early `selectProjectById` lookup so a concurrent rescan can't orphan the entry between lookup and append. `JournalViewModel.pathHintFor(entry)` prefers the denorm column, falling back to the action variant's payload.

Already-orphaned rows from before this PR ships (5 such rows in the user's screenshot) are unrecoverable — no path data exists for them anywhere — and will keep showing the sentinel until they age out of `LIMIT 200`.
...
3. `feat(catalog): backfill journal_entries.project_name (8.sqm)` — name backfill + probe bump.
4. `feat(catalog): denormalize project_path on journal_entries (9.sqm)` — path column + backfill, schema + insert/select + probe.
5. `feat(repo,journal): use project_path as pathHint fallback` — repair callsites pre-fill name+path; viewmodel prefers denorm column over action-derived path.

Plus PR-review fixups (probe `else -> 7L`, mapKeys allocation hoist, design typos) and a baseline regen.
