...
**CI auth via Workload Identity Federation, not JSON keys.** GitHub Actions mints an OIDC token; GCP exchanges it for a 1-hour access token bound to the release-uploader SA. The OIDC provider's attribute condition restricts impersonation to repos owned by `srMarlins`, and the SA's `iam.workloadIdentityUser` binding restricts it further to `srMarlins/sketchbook`. No long-lived service-account key exists for the release pipeline (the org policy `iam.disableServiceAccountKeyCreation` is on, and WIF is the better practice regardless). The only secret in CI is `CONVEYOR_SIGNING_KEY` — that one is the actual root of trust.
