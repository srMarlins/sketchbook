...
3. **Call `auth.revokeRefreshTokens(uid)` on sign-out.** Today we revoke the Google OAuth grant. With Firebase we should *also* hit Firebase's [revoke endpoint](https://firebase.google.com/docs/auth/admin/manage-sessions#revoke_refresh_tokens) — guarantees the refresh token is unusable even if a malicious process held a copy. Closes the race window between sign-out and keyring deletion. **Split across phases:** Phase 2 implements the local-state portion (clear keyring + in-memory cache under a mutex so concurrent readers can't observe stale tokens). The Admin SDK side is server-only (Identity Toolkit has no public client-facing revoke endpoint), so a `revokeMySession` Cloud Function lands in Phase 3 alongside the MetadataStore deployment.
