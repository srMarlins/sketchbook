...
/**
 * In-memory [CloudBackend] for integration tests. Tracks blobs by hash, manifests per tree, and
 * a single `head` per tree. Conditional writes use a synthetic monotonically increasing
 * generation counter.
 */
class FakeCloudBackend : CloudBackend {
    ...
    private data class TreeKey(val treeId: TrackedTreeId, val kind: TrackedTreeKind)
    ...
    override suspend fun readManifest(treeId: TrackedTreeId, kind: TrackedTreeKind, rev: SnapshotRev): Manifest {
        val list = manifests[TreeKey(treeId, kind)]
    ...
    fun headGenerationFor(uuid: ProjectUuid): Generation = manifests[projectKey(uuid)]?.lastOrNull()?.ref?.generation ?: Generation.ZERO
