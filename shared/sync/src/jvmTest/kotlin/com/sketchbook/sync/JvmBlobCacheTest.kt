...
class JvmBlobCacheTest {
    ...
    @Test
    fun fetchOnMissThenServeFromCacheOnHit() =
        runTest {
    ...
    @Test
    fun separateScopesAreSeparateCacheKeys() =
        runTest {
            val cloud = CountingCloud(payload)
            val handle = CatalogDb.openInMemory()
            val cache = JvmBlobCache(handle.catalog, tmp, cloud, cacheSettings = BlobCacheSettings.Default)
    ...
    @Test
    fun fetchStreamsLargeBlobWithoutHeapBuffering() =
        runTest {
...
private class CountingCloud(
    ...
    // Lock methods removed in Phase 3 — see InMemoryMetadataStore.
}
