...
class WatcherToSyncStateTest {
    ...
    @Test
    fun `als save under known project marks dirty`() =
        runBlocking {
    ...
    @Test
    fun `als save under unknown directory does NOT mark dirty`() =
        runBlocking {
            val handle = CatalogDb.openInMemory()
            val catalog = handle.catalog
            val syncState = SyncStateStore(catalog)
    ...
    @Test
    fun `backup folder save resolves to parent project dir`() =
        runBlocking {
            val handle = CatalogDb.openInMemory()
            val catalog = handle.catalog
            val syncState = SyncStateStore(catalog)
