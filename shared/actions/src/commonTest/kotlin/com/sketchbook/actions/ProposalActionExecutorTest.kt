...
class ProposalActionExecutorTest {
    ...
    private class FakeProjects : ProjectRepository {
        data class ArchiveCall(val id: ProjectId, val archived: Boolean)
        ...
        private fun stubEntry(): JournalEntry = JournalEntry(
    ...
    @Test
    fun archiveActionDispatchesToRepoArchive() = runTest {
    ...
    @Test
    fun unknownActionTypeFailsWithoutPartialEffects() = runTest {
        val fake = FakeProjects()
        val exec = ProposalActionExecutor(fake)
        val r = exec.apply(
            listOf(
